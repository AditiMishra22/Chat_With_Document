...
Open PowerShell, then change into the demo directory:
...
Create a .env file in the same folder as app.py containing at least the GROQ API key (required for the ChatGroq model):
GROQ_API_KEY=your_groq_api_key_here
# Optional: override Flask session secret key
SECRET_KEY=your_secret_here
Run the app:
python app.py
Open your browser at http://127.0.0.1:5000 — upload PDF(s) and ask questions.
API endpoints & usage
...
The app loads PDF files using PyPDFLoader (paginated documents).
Documents are split into chunks with RecursiveCharacterTextSplitter (chunk_size=800, overlap=100).
Each chunk is embedded using sentence-transformers/all-MiniLM-L6-v2 (HuggingFaceEmbeddings).
A FAISS in-memory vector index is created from the embeddings.
For each user query the code builds a retriever (k=6), runs a retrieval chain to gather context, and invokes the ChatGroq model to generate an answer constrained to the supplied context via a ChatPromptTemplate.
